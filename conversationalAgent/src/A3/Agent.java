		...
		while(!done) {			//loop begins
			try {
				...
				if(name.toLowerCase().contains("my name is")) {
					...
					out.writeUTF("AGENT: Nice to meet you " + name.substring(0, 1).toUpperCase() + name.substring(1) + ". Alice 2.1 at your service.");
				}
				else
					out.writeUTF("AGENT: Nice to meet you " + name.substring(0, 1).toUpperCase() + name.substring(1) + ". Alice 2.1 at your service.");
...
//			    	String namedResponse = getNamedResponse(msgNonEdited);
					...
					if(msg.equalsIgnoreCase("close") || msg.equalsIgnoreCase("quit")) {	//for terminating connection with client
					...
					else if( msg.toLowerCase().startsWith("what is")) {
						out.writeUTF(askWiki(msg.substring(8)));
...
//					}
	...
	public static String askWiki( String question) throws IOException, InterruptedException {
		...
		HttpRequest request = HttpRequest.newBuilder(
		       URI.create("http://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=" +question+ "&format=json"))
		...
		JsonArray array = jsonReader.readObject().getJsonObject("query").getJsonArray("search");
		

		for (JsonValue o : array) {
			String title = ((JsonObject) o).getString("title");	
			String link = "https://en.wikipedia.org/wiki/" + title;	
			String rawOutput = ((JsonObject) o).getString("snippet");
